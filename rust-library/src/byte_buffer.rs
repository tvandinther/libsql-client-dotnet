...
impl ByteBuffer {
    ...
    pub fn from_vec_of_pointers(v: Vec<*mut ByteBuffer>) -> Self {
        let length = (v.len() * std::mem::size_of::<*mut ByteBuffer>()) as i32;
        let capacity = (v.capacity() * std::mem::size_of::<*mut ByteBuffer>()) as i32;
        let ptr = Box::into_raw(v.into_boxed_slice()) as *mut ByteBuffer as *mut u8;
        Self { ptr, length, capacity }
    }
