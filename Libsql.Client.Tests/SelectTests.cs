...
public class SelectTests : IDisposable
{
    ...
    {
        await _db.Query("CREATE TABLE IF NOT EXISTS test (id INTEGER, value INTEGER)");
        await _db.Query("INSERT INTO test (value) VALUES (NULL)");
        
        var rs = await _db.Query("SELECT value FROM test");
        var row = rs.Rows.First();
        var value = row.First();
    ...
    {
        var rs = await _db.Query("SELECT 'Hello, World!'");
        var row = rs.Rows.First();
        var value = row.First();
    ...
    {
        var rs = await _db.Query("SELECT 'Привет, мир!'");
        var row = rs.Rows.First();
        var value = row.First();
        var text = Assert.IsType<Text>(value);
    ...
    {
        await _db.Query("CREATE TABLE IF NOT EXISTS test (id INTEGER, value TEXT)");
        await _db.Query("INSERT INTO test (value) VALUES (NULL)");
    ...
    {
        await _db.Query("CREATE TABLE IF NOT EXISTS test (id INTEGER, value REAL)");
        await _db.Query("INSERT INTO test (value) VALUES (NULL)");
    ...
    public async Task SelectBlobType()
    {
        ...
        var rs = await _db.Query("SELECT X'1e092a4c'");
        var row = rs.Rows.First();
        var value = row.First();
    ...
    {
        await _db.Query("CREATE TABLE IF NOT EXISTS test (id INTEGER, value BLOB)");
        await _db.Query("INSERT INTO test (value) VALUES (NULL)");
